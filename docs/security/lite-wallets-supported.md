...
> **Note:** “Lite wallets” here means using **remote blockchain nodes** (no local full node)—**not** importing external Bitcoin watch-only wallets (xpub / view-key). External xpub-only wallets are **not** supported; see [FAQ — Lite wallets supported?](../faqs/index.md#lite-wallets-supported).
