...
The merchant endpoint must return **HTTP 202**. The current callback code treats any other code as failure (`callback.py`).

## Invoice callbacks

Unconfirmed txs with `callback_confirmed=false` are retried by `send_callbacks`. Confirmed txs wait `NOTIFICATION_TASK_DELAY` (default 60s), then POST. Failures stay `callback_confirmed=false` and are tried again every 60 seconds. There is no `MAX_RETRIES` cap and no backoff on invoice callbacks.
